...
% directory of windows explore.
% [filename,path_folder] = get_old_folder_position(varargin)
...
date_today = c(1:3);
if isgnu()
    ex_def_path = exist(strcat(pwd,'/default_path_gnu.mat'),'file');
else
...
if isempty(extension)
    ...
    switch ex_def_path
        case 0
            ...
            if isgnu()
                save('default_path_gnu.mat','path_folder_saved','date_saved','extension_saved');
            else
                save('default_path.mat','path_folder_saved','date_saved','extension_saved');
            end
        case 2
            if isgnu()
                load('default_path_gnu.mat','path_folder_saved','date_saved','extension_saved');
            else
            ...
            if strcmp(path_folder(end),'/') || strcmp(path_folder(end),'\')
                path_folder_saved = path_folder;
            else
    ...
    switch ex_def_path
        case 0
            ...
            if isgnu()
                save('default_path_gnu.mat','path_folder_saved','date_saved','extension_saved','filename_full_saved');
            else
                save('default_path.mat','path_folder_saved','date_saved','extension_saved','filename_full_saved');
            end
        case 2
            if isgnu()
                tmp = load('default_path_gnu.mat');
            else
            ...
            if isfield(tmp,'filename_full_saved')
                clear tmp
                if isgnu()
                    load('default_path_gnu.mat','path_folder_saved','date_saved','extension_saved','filename_full_saved');
                else
